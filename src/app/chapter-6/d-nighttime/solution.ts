...
export class NighttimeComponentSolution {
   ...
   */
  summonMoon() {
    const subscr = Observable.combineLatest(this.getNighttimeParts(), this.getOtherNighttimeParts())
      .subscribe(
        nightTimePartArray => {
          if (nightTimePartArray.every(elem => elem === undefined)) {
            subscr.unsubscribe();
          } else {
            if (nightTimePartArray[0] && this.nighttimeAssembled.indexOf(nightTimePartArray[0]) === -1) {
              this.nighttimeAssembled += nightTimePartArray[0] + " ";
            }
            if (nightTimePartArray[1] && this.nighttimeAssembled.indexOf(nightTimePartArray[1]) === -1) {
              this.nighttimeAssembled += nightTimePartArray[1] + " ";
            }
          }
        }
      );
